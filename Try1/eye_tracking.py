...
# Keep track of the gaze positions and their frequencies
gaze_positions = {}
...
while(True):
    ...
    for rect in rects:
        ...
        gaze_x = (shape[39][0] + shape[42][0]) // 2
        gaze_y = (shape[38][1] + shape[43][1]) // 2
        
        gaze_position = (gaze_x, gaze_y)
        gaze_positions[gaze_position] = gaze_positions.get(gaze_position, 0) + 1
        
    max_frequency = max(gaze_positions.values()) if gaze_positions else 1
    for position, frequency in gaze_positions.items():
        x, y = position
        intensity = int(255 * (frequency / max_frequency))
        cv2.circle(heatmap, position, 10, (intensity, 0, 0), -1)
        
    combined = cv2.addWeighted(img, 0.7, cv2.applyColorMap(heatmap, cv2.COLORMAP_JET), 0.3, 0)
